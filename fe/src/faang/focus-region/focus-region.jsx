...
export function _FocusRegion({
      ...
      if (a !== E) {
        if (E.restorationFocusRegionItem !== a) {
          ...
          if (
            ...
            !(!(b = z.current) ? undefined : b.containsNode(a.lastFocused))
          ) {
        ...
        (!a || (a && E && a.lastFocused !== E.lastFocused)) &&
          B.setActiveFocusRegion(E);
      }
    }
  }, [B, E]);
      ...
      return {
        ...
        onAfterBlurWithin: () => {
          ...
          if (
            ...
            let h = FocusManager.getAllNodesFromOneOrManyQueries(
              recoverFocusQuery,
              a
            );
            if (h && f) {
              ...
              for (var k = g - 1; k >= 0; k--) {
                ...
                if (i.has(l)) {
                  ...
                  if (m < h.length) {
                    ...
                    if (!j.has(m)) {
                      b.detachedCanTab &&
                        setElementCanTab.setElementCanTab(m, true);
                      deferredFocusElement(m, c, e);
                      return;
                    }
                  }
                  b.detachedCanTab &&
                    setElementCanTab.setElementCanTab(l, true);
                ...
                for (m = g + 1; m < f.length; m++) {
                  ...
                  if (i.has(l)) {
                    ...
                    if (k >= 0) {
                      g = h[k];
                      b.detachedCanTab &&
                        setElementCanTab.setElementCanTab(g, true);
                      deferredFocusElement(g, c, e);
                      return;
                    }
                  }
                }
              l = FocusManager.getFirstNodeFromOneOrManyQueries(
                recoverFocusQuery,
                a
              );
              l &&
  ...
  ReactKeyboardEvent.useKeyboard(
    ...
    useMemo(() => {
      return {
        onKeyDown: (a) => {
          if (!containFocusQuery || a.key !== "Tab" || a.isDefaultPrevented())
            return;
          let b = z.current;
          b &&
            (a.shiftKey
